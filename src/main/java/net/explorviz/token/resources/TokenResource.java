  ...
  public Response modifyAccessToToken(@PathParam("tid") final String tokenId,
    ...
    if (token.isPresent()) {
      if ("revoke".equals(method)) {
        ...
        this.tokenService.cloneToken(tokenId, userId, token.get().getAlias(), token.get().getIsRequestedFromVSCodeExtension(), token.get().getProjectName(), token.get().getCommitId());
      }
      return Response.noContent().build();
    } else {
